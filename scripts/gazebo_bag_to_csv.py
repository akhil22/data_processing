...
field_names = ['vx','pedal_c','brake_c','steer_val','steer_vel_out','steer_cmd','v_out','omega_out','accel_out']
...
del_t = 0.02
got_first_msg = False
def fill_prev_data(steer_msg, pedal_command_msg, vel_msg, brake_cmd_msg):
    global got_first_msg, prev_steering, prev_vel, prev_pedal_cmd, prev_steering_cmd, prev_brake_cmd
...
def callback(vel_msg, pedal_command_msg, accel_msg, steer_msg, brake_cmd_msg):
    global got_first_msg, prev_steering, prev_vel, prev_vel_cmd, prev_steering_cmd, del_t
    if not got_first_msg:
        got_first_msg = True
        fill_prev_data(steer_msg, pedal_command_msg,  vel_msg, brake_cmd_msg)
        return
    else:
       #print((steer_msg.steering_wheel_angle - prev_steering)/del_t)
       #prev_steering = steer_msg.steering_wheel_angle
       data_writer.writerow({'vx': prev_vel, 'pedal_c': prev_pedal_cmd, 'brake_c': prev_brake_cmd, 'steer_val': prev_steering, 'steer_vel_out': (steer_msg.steering_wheel_angle - prev_steering)/del_t, 'steer_cmd': prev_steering_cmd,'v_out': vel_msg.twist.linear.x, 'omega_out': vel_msg.twist.angular.z, 'accel_out': accel_msg.data})
       fill_prev_data(steer_msg, pedal_command_msg, vel_msg, brake_cmd_msg)
       print("filled_data")
